module.exports = function (ctf) {
    ...
    var client = storage.bucket(process.env.GCS_BUCKET);
    ...
    router.delete("/:file", async (req, res) => {
        var f = await File.findOne({
            challenge: parseInt(req.params.challenge),
            id: parseInt(req.params.file),
        });
        if (!f) return res.json({message: "file_not_found"});
        try {
            await client.file(f.path).delete();
        } catch {}
        await File.delete({
            challenge: parseInt(req.params.challenge),
            id: parseInt(req.params.file),
    ...
    router.post("/", upload.single("file"), async (req, res) => {
        var c = ctf.models.Challenge.findOne({
            competition: req.competition,
            id: req.params.challenge,
        });
        if (c) {
            var hash = crypto
                .createHash("sha256")
        ...
        if (!req.user || !req.user.admin) {
            ...
            for (var c = 0; c < challenges.length; c++) {
                ...
                for (var f = 0; f < files.length; f++) {
                    challenges[c].description = challenges[
                        c
                    ].description.replace(
                        new RegExp(
                            "\\[(.*?)\\]\\(" +
