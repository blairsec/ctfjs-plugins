module.exports = function (ctf) {
    ...
    async function sendVerificationEmail(email, user, competition) {
        ...
        for (var r = 0; r < emails.length; r++) {
            if (new Date().getTime() - new Date(emails[r].expiry).getTime() < 0)
                throw {message: "email_already_sent"};
        }

        var token = crypto.randomBytes(16).toString("hex");
        var expiry = new Date(new Date().getTime() + 15 * 60000);
        var reset = await new EmailVerification({
            token: token,
            expiry: expiry,
            user: u.id,
            email: email,
...
You recently signed up for ${h.title} ${
                c.name
            }. To confirm your email and activate your account, please visit the following link: ${
                process.env.VERIFY_URL.replace(/<competition>/g, c.name) + token
            }
...
- ${h.title} Team`,
            html: `<div>Hello <code>${u.username
                ...
                .replace(/>/g, "&gt;")}</code>,</div>
<div><br></div>
<div>You recently signed up for ${h.title} ${
                c.name
            }. To confirm your email and activate your account, please visit the following link: <a href="${
                process.env.VERIFY_URL.replace(/<competition>/g, c.name) + token
            }">${
                process.env.VERIFY_URL.replace(/<competition>/g, c.name) + token
            }</a></div>
...
<div>- ${h.title} Team</div>`,
    ...
    router.post(
        ...
        async (req, res) => {
            ...
            var verification = await EmailVerification.findOne({
            ...
            if (
                new Date().getTime() -
                    new Date(verification.expiry).getTime() >=
                0
            )
                return res.status(404).json({message: "token_not_found"});
    ...
    router.get(
        ...
        async function (req, res) {
            var pending = await EmailVerification.find({
                user: req.user.id,
                used: false,
            });
            var results = [];
            for (var p = 0; p < pending.length; p++) {
                if (
                    new Date().getTime() -
                        new Date(pending[p].expiry).getTime() <
                    0
                )
                    results.push({
                        email: pending[p].email,
                        expiry: pending[p].expiry,
                    });
            }
