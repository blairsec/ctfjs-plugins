module.exports = function (ctf) {
  ...
  var transporter = nodemailer.createTransport({
    ...
    var u = await ctf.models.User.findOne({email: req.body.email, competition: c.id})
    ...
    for (var r = 0; r < resets.length; r++) {
      if (new Date().getTime() - new Date(resets[r].expiry).getTime() < 0) return res.status(409).json({ message: 'password_reset_already_active' })
    }

    var token = crypto.randomBytes(16).toString('hex')
    var expiry = new Date(new Date().getTime() + 15*60000)
    var reset = await new PasswordReset({ token: token, expiry: expiry, user: u.id })
    await reset.save()
...
You recently requested to reset your password for ${h.title} ${c.name}. To reset your password, please visit the following link: http://2018.angstromctf.local/resetpassword?token=${token}
...
<div><br></div>
<div>You recently requested to reset your password for ${h.title} ${c.name}. To reset your password, please visit the following link: <a href="http://2018.angstromctf.local/resetpassword?token=${token}">http://2018.angstromctf.local/resetpassword?token=${token}</a></div>
<div><br></div>
...
<div>- ${h.title} Team</div>`
    ...
    var reset = await PasswordReset.findOne({ token: req.query.token, used: false })
    if (!reset) return res.status(404).json({message: 'token_not_found'})
    if (new Date().getTime() - new Date(reset.expiry).getTime() >= 0) return res.status(404).json({message: 'token_not_found'})
    ...
    var reset = await PasswordReset.findOne({ token: req.body.token, used: false })
    if (!reset) return res.status(404).json({message: 'token_not_found'})
    if (new Date().getTime() - new Date(reset.expiry).getTime() >= 0) return res.status(404).json({message: 'token_not_found'})
    reset.used = true
    await reset.save()
